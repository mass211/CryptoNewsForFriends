...
session = requests.Session()  # بهبود سرعت درخواست‌ها
...
if __name__ == "__main__":
    ...
    new_news = fetch_and_send_all()

    if not new_news:
        elapsed = (datetime.datetime.now() - start_time).total_seconds()
        wait_time = max(0, 120 - elapsed)
        print(f"خبری نبود، منتظر {wait_time:.1f} ثانیه می‌مانم تا ۲ دقیقه کامل شود.")
        time.sleep(wait_time)
    else:
        print("خبر جدید ارسال شد، برنامه سریع پایان می‌یابد.")
